...
- SQLite `painting_file_ref` table (one row per referenced file)
...
- Output / input file ids do **not** live on the painting row. They are emitted
  as `painting_file_ref` rows: `sourceId=painting.id`, `role='output' | 'input'`
- `painting_file_ref` emission is **pre-filtered against `file_entry`**: ids the
...
- The JSON files column (replaced by `painting_file_ref` rows)
