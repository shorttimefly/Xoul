...
- 产品形象：产品配置支持 PNG/JPG/WebP 上传、预览、替换和移除；上限 2MB、1600 万像素，保存为最长边 1200px 的本地图片。保存产品后会异步调用已选公共模型进行图片理解，读取清晰可见文字并核验是否能确认知名品牌；生成主体、场景、使用方式、安全边界及可编辑的产品 Prompt，确认的品牌信息会补入 Prompt。仅当产品 Prompt 为空时自动填入识别出的 Prompt，避免覆盖人工设定。当前图片仅存于当前浏览器，不提供跨设备图片服务。
