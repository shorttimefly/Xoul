(function () {
  ...
  function understandingText(result){const brand=result.brand||{};return [['主体',result.subject],['场景',result.scene],['明显文字',Array.isArray(result.visible_text)?result.visible_text.join('、'):result.visible_text],['品牌确认',brand.is_known&&brand.name?brand.name+(brand.evidence?'（'+brand.evidence+'）':''):'未确认知名品牌'],['使用场景',Array.isArray(result.use_cases)?result.use_cases.join('、'):result.use_cases],['适用人群',Array.isArray(result.suitable_for)?result.suitable_for.join('、'):result.suitable_for],['使用方法',result.usage_method],['安全提示',Array.isArray(result.safety)?result.safety.join('、'):result.safety],['产品 Prompt',result.product_prompt]].filter(([,value])=>value).map(([label,value])=>label+'：'+value).join('\n');}
